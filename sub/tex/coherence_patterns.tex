...
\newcite{parveen15} propose a summarizer that models the extraction of sentences as an optimization problem in that a summary is a set of selected sentences that optimize the all aforementioned factors. 
...
The notation $D=\lbrace \left( d_i, s_i \right) \rbrace$ refers to a dataset consisting of a set of document-summary pairs where the $i^{th} pair contains document $d_i$ and summary $s_i$.  
Assume that summary $s_i$ is a gold summary for document $d_i$. 
Gold summaries can be used to train a summarization model, and can also be used to evaluete  summaries produced by the model duting the evalution phase. 
