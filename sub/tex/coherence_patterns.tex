...
The graph representation of the distribution of entities in a text is then transformed into a one-mode projection graph among sentence nodes. 
...
\newcite{guinaudeau13} assume that the average outdegree quantifies the local coherence of a text. 
...
If nodes in a graph are ordered or relations between nodes are not symmetry then pairs in the edge set  of the graph should be interpreted as directed edges. 
...
\begin{figure}[!ht]
	\begin{center}
		\begin{tikzpicture}
			\node[] (dataset1) at (0,10)  {\includegraphics[scale=0.10]  {../figures/text.jpeg}};
			\node[] (eg1) 	   at (0,8)   {\includegraphics[scale=0.03] {../figures/bg1.png}};
			\node[] (proj1)    at (0,6.2) {\includegraphics[scale=0.03] {../figures/proj1.png}};
			...
			\node[] (dataset2) at (3,10)  {\includegraphics[scale=0.10]{../figures/text.jpeg}};
			\node[] (eg2) 	   at (3,8)   {\includegraphics[scale=0.03]{../figures/bg1.png}};
			\node[] (proj2)    at (3,6.2) {\includegraphics[scale=0.03]{../figures/proj1.png}};
			...
			\node[] (dataset3) at (9,10)  {\includegraphics[scale=0.10]{../figures/text.jpeg}};
			\node[] (eg3) 	   at (9,8)	  {\includegraphics[scale=0.03]{../figures/bg1.png}};
			\node[] (proj3)    at (9,6.2) {\includegraphics[scale=0.03]{../figures/proj1.png}};
			\path[->, thick] (dataset3) edge  (eg3);
			\path[->, thick] (eg3) edge  (proj3);
			
			
			\node[] (minesg) at (5,4.2) {\includegraphics[scale=0.3]{../figures/3node.jpg}};
...
We use the dataset collected by \newcite{pitler08} which consists of thirty articles randomly selected from the Wall Street Journal corpus. 
...
We follow \newcite{pitler08} and treat this as a classification problem such that given a text pair, which text is easier to read?
...
We employ the summarization proposed by \newcite{parveen15a} due to it is based on the entity graph model (see Chapter \ref{ch:rel-work}), which is the same framework as our coherence model. 
\newcite{parveen15a} assume that the outdegree of each node in the projection graph of an input text measures how crucial the sentence that is corresponded with the node is for coherence of the summary.  
...
\begin{itemize}
	...
	\item 
	In the summarization model proposed by \newcite{parveen15a}, the outdegrees of sentence nodes are computed in the projection graph of an input text.   
...
Similarly if an entity is selected to be mentioned in the summary then at least a sentence that contains a mention of the entity is selected as well:   
...
The summarizer that is enriched by our coherence pattern outperforms \emph{Mead} as one of the state-of-the-art summarization systems. 
