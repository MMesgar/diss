...
\begin{tabular}{ccc}
	...
	\tikzstyle{edge}=[draw=black!90, thick]
	 ...
	 \node [node] (3) at (0,0) {\small{$4$}}; 
	 \node [node] (4) at (4,0) {\small{$3$}}; 
  ...
  \end{tikzpicture}
	 ...
	 \node  (1) at (0,4) {$f(a) = 1$};
	 \node  (2) at (0,2.7) {$f(b) = 4$};
	 \node  (3) at (0,1.3) {$f(c) = 2$}; 
	 \node  (4) at (0,0) {$f(d) = 3$}; 
...
\end{tabular}
\caption{Two isomorphic graphs and an illustration between its nodes. }
\label{fig:isomorphic_graph}

\end{figure} 

\textbf{Subgraph.} Graph $G^\prime$ is a \textit{subgraph} of graph $G$, if $G^\prime$ is isomorphic to a graph whose nodes and edges are in $G$.


$\mathit{k}$\textit{-node subgraph.} A subgraph with $k$ nodes is called $\mathit{k}$\textit{-node subgraph}.
...
% if $G^\prime$ has all edges of $G$, which their corresponding nodes
% are in $G^\prime$ 
(Figure \ref{f:induced_subgraphs}). 
We always mean induced subgraphs when using the term subgraph.  
    
%\input{figures/induced_subgraphs}
