...
The average outdegree of nodes in projection graphs measures the extent to which sentence nodes in a projection graph are connected to each other. 
...
The results of the experiments in this chapter support this hypothesis (see Section \ref{sec:exp}). 
...
%because it contains all possible edges that are present in $G_1$ and connect the nodes that are associated with nodes in $G_3$.  
...
It is worth mentioning that in the research of this thesis we mean induced subgraphs when we use the term ``subgraph''.  
...
One-mode projection graphs model the connectivity between sentence nodes considering the shared entities between sentences. 
...
The connectivity of each graph in a graph set can be represented by its graph signature.
...
That is the reason that these basis graphs are referred to as coherence patterns, and the frequencies of them, which are encoded in the signature of the graph, is taken as coherence features. 
...
In the research of this thesis, the readability assessment task mainly is utilized to evaluate the impact of coherence features in quantifying the difficulty of texts.  
In a coherent text, each sentence has some connections with other sentences. 
Although these local connections somewhat make texts easy to read, the coherence features introduced by the entity grid model \cite{barzilay08} are not strongly correlated with readability ratings assigned by human judges \cite{pitler08}.  
...
That is why we do not use coherence features to predict the exact readability scores associated with texts. 
...
Each of the judges had unlimited time to read the texts and perform the ratings. 
...
The text presented in Example \ref{ex:WSJ-1818} is one\footnote{The article's ID is WSJ-1818.} of the articles in this dataset with the final human readability score of 3.7 out of five.  
...
The goal of this experiment is to investigate the correlation of different coherence features with readability ratings associated with the articles. 
...
Having projection graphs of every article in the dataset, we compute the Pearson correlation between the values of a feature and readability ratings associated with articles. 
The reason for using the Pearson correlation is that we assume that feature values and ratings are meaningful by themselves. 
...
The entity graph implementation is performed in Java and is publicly available on the Github of the author of this thesis.
...
These 4-node subgraphs have more capacity to capture connectivity information than 3-node subgraphs because 4-node patterns contain more nodes and therefore more possible edges. 
However, 4-node subgraphs are less likely to occur in projection graphs.
So they are expected to distinguish texts better.  
...
From the texture perspective, the frequency of each subgraph models how frequently a coherence pattern is used in a text. 
...
First, the number of 4-node patterns whose frequencies significantly correlate with readability ratings is more than those in 3-node patterns. 
...
While these features are intended to capture entity-based coherence, entity transitions in isolation seem to be too weak to do so. 
...
The first set of coherence features is grammatical transitions of entities, with a sequence length of two, proposed by the entity grid model \cite{barzilay05a,barzilay08}. 
...
We use the notation $H$ to denote the dataset that consists of summaries written by humans for a set of texts.  
...
Moreover, summarizing scientific articles is difficult as multi-document summarization because scientific articles tend to be long and the important information is spread all over an article,  unlike the distribution of information in a news article \cite{teufel02}. 
...
Abstracts of scientific articles can be taken as summaries of articles as well, which are coherent because humans write them. 
...
Moreover, the outdegree does not capture how sentences should be connected so that the summary would be coherent. 
...
Applying the $\chi^2$ statistical test shows that W is statistically significant (p\_value <.05).  It indicates that the rankings provided by the human judges are reliable and informative.
...
In the former, we observed that frequencies of some coherence patterns are positively and some others negatively correlate with readability ratings, which are assigned to texts by human judges. 
...
Our \mbox{data-driven} approach to coherence pattern mining enables our model to extract patterns from documents so that the patterns are beneficial for the desired application. 
...
Besides, it is also restricted to only noun overlap relation among nouns in sentences in a text, while any lexical semantic relation between words in sentences can relate sentences. 
