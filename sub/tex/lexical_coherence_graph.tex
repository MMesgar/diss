...
We explain the sparsity problem related to the frequencies of subgraphs and then adapt a solution from statistical language models for this problem for graph-based purposes (Section \ref{sec:lex-graph-smoothing}). 
...
Following is some examples of such relations: 
...
\item Collocation which happens when two words frequently occur in the same context together. As an example, words ``doctor'' and ``patient'' are semantically related just because they frequently are used together.

\end{itemize}

An important property of lexical relations is that lexical items should not necessarily have the same reference \cite{halliday76} in order to relate two sentences.
...
For instance, almost none of the available non-English WordNet resources comes close to the WordNet's size and coverage on English. 
...
Embedding representations of words give the model the capability of efficiently computing semantic relations among lexical items in a vector space. 
...
It is worth mentioning that such word vectors can be easily trained for any language if a large corpus of texts on the language is available.  
...
\newcite{halliday76} argue that for the texture purposes, it is only necessary to recognize lexical items that are semantically related. 
There is coherence between any pair of lexical items that stand to each other in some lexicosemantic relation, regardless of their relation type. 
...
In the research of this thesis, we refer to this graph representation of a text as lexical cohesion graph or \emph{LexGraph}. 

More formally, LexGraph $G=<V,E>$ comprises of two sets: $V$ is a set of nodes representing sentences in a text, and $E$ is a set of directed edges. 
...
The result is a graph in which every two nodes are connected by weighted edges.    
...
From the computational point of view, this parameter makes the graphs sparse so that the differences in the structure of the graph can be distinguished by the model. 
...
The features capture the connectivity style of a graph and, consequently, the coherence of the corresponding text.   
...
We need to define them only once, so the processing time of this step does not have any negative impact on the overall efficiency of the mining method.  
...
Function $\mathtt{GetID}$ compares its input subgraph with each pattern in the pattern-list and returns the index of the pattern that is isomorphic with the input subgraph.  
...
The complexity of the method presented in Algorithm \ref{alg:pattern-counting} is $\mathcal{O}(N^ \ast M)$  where $N$ is the number of graphs (i.e.\ the number of texts in a corpus) and $M$ is the maximum number samples that should be drawn from each graph, which is shown by $\mathtt{MAX}$ in Algorithm \ref{alg:pattern-counting}.
...
Similarly, the sparsity issue in graphs can also be solved by a smoothing technique.  
...
It estimates the frequency of a subgraph, which does not occur in a graph, based on the frequencies of subgraphs that are smaller than the subgraph. 
This estimation allows the model to measure the coherence of a text even where its corresponding graph representation does not contain some subgraphs. 
...
The introduced sampling approach for subgraph mining in this Chapter efficiently supports this requirement. 
...
\end{figure}
%
As an instance, consider pattern $p_1$, pattern $p_2$, and pattern $p_5$ in Figure \ref{fig:parent-child-rel}. 
...
We extend the entity grid representation of texts by incorporating the connections between pronouns and their antecedents. 
...
In this way, we prevent the model noisy edges, which  are not strong enough in terms of the cosine similarity, to be added in graphs. 
...
We check the impact of the size of subgraphs by evaluating patterns with different number of nodes $k \in \lbrace3,4,5,6\rbrace$. 
...
The best value for the discount factor in Equation \ref{eq:knser-ney}, i.e.\ $\alpha$, is obtained in a greedy fashion and iterative way.   
...
We first evaluate how LexGraph representation of texts performs in comparison with the entity graph representations.  
We then assess the influence of the smoothing approach on the performance of the LexGraph model. 
We finally evaluate the quality of the several coherence patterns which are extracted from the LexGraph representations of texts and compare them with patterns which are extracted from the entity graph representations of texts. 

\paragraph{Evaluating LexGraph representations.} 
Here we evaluate LexGraph representations of texts in the \pitlerds dataset. 
...
The frequencies of coherence patterns in a text is taken as features, which represent the coherence of the text. 
...
Large patterns can lead to features that are highly predictive for discriminating coherence texts from incoherent ones. 

Given 3-node patterns the LexGraph model does not beat the EGraph mode. 
Considering \mbox{4-node} patterns, the performance of LexGraph is closer to the performance of the EGraph model. 
...
We use 5-node patterns because these can distinguish between texts better than other pattern, given any of two examined graph representations of texts, i.e.\ the entity graph and the LexGraph representations. 
...
The problem with such vectors is that each graph representation becomes unique in a dataset and a machine learning model cannot learn from similarities and dissimilarities between graphs. 
...
When we apply Kneser-Ney smoothing the results for all examined values of $k$ improves on this dataset.  
...
We believe that this approach provides more informative graph representations of texts for coherence modeling in comparison to the entity graph representation of texts because the relations between sentence in LexGraph are not limited to coreference relation among named entities. 
