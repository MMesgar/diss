...
This is achieved through cohesive semantic relations that are expressed through the grammar and the vocabulary of a language \cite{halliday76}. 
...
In the research in this dissertation, we refer to this graph representation of a text as a lexical cohesion graph or \emph{LexGraph}. 
...
Ideally, if a sufficient number of sample subgraphs are drawn from a graph, then the empirical distribution is close to the actual distribution of patterns in the graph. 
...
However, many large subgraphs do not occur in a graph representation of a text yielding a sparsity problem in feature vectors. 
...
We can observe from the results that both the entity graph model, i.e.\ EGraph and the LexGraph model rank texts with respect to their coherence superior to the EGrid model \cite{barzilay08}.  
This observation confirms our initial intuition in the research in this thesis that graph-based models have more capacity to encode relations among sentences in a text because graphs can capture long-distance relations, which is informative for coherence modeling.  
...
Since the LexGraph is dense, 3-node subgraphs cannot encode the difference between the structure of them. 
...
The other major difference between these two datasets is that the articles in the \declercqds dataset are from different genres, but those in the \pitlerds dataset are only from the newswire. 
...
The lowest performance of both models is obtained from texts that are Manuals; however, the LexGraph model with a large margin outperforms the EGraph model on this genre as well. 
...
On the other hand, large subgraphs occur in a few graphs in a dataset. 
So when large subgraphs are taken into account, each graph is represented by a large vector because there are many possible subgraphs, where most of the elements in the vector are zero.   
...
It shows that our coherence patterns plus the LexGraph representation of a text construct a model for local coherence modeling. 
...
However, this improvement is not as high as the obtained improvement on LexGraph.
These results show that our smoothing method is useful for both graph-based models, and our new graph representation is more informative than the entity graph model for coherence modeling. 
